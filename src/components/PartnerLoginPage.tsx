...
import { Handshake, Lock, Mail, ArrowRight, Home } from 'lucide-react';
    ...
    try {
      ...
      if (authData.user) {
        ...
        if (!partner) {
          ...
          throw new Error('Partner account not found. Please contact support.');
        }

        if (partner.status !== 'active') {
          await supabase.auth.signOut();
          throw new Error('Your partner account is not active. Please contact support.');
        }
        ...
        <div className="bg-white rounded-2xl shadow-xl border border-slate-200 p-8">
          ...
          <form onSubmit={handleLogin} className="space-y-6">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-gradient-to-r from-emerald-500 to-emerald-600 text-white py-3 rounded-lg font-medium hover:from-emerald-600 hover:to-emerald-700 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center space-x-2"
            >
